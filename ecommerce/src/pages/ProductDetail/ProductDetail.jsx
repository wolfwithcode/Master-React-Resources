...
export default function ProductDetail() {
  ...
  // phải destructuring, vì mỗi lần re-render thì
  // useParams lại trả về 1 object mới
  const { idProduct } = useParams()
  const dispatch = useDispatch()
  useEffect(() => {
    const realId = getIdFromNameId(idProduct)
    http.get(`products/${realId}`).then(res => {
  ...
  useEffect(() => {
    if (product) setCurrentImages(product.images.slice(...currentIndexImages))
  ...
  const handleAddToCart = async () => {
    try {
      ...
      await purchaseApi.addToCart(body)
      ...
      {product && (
        <div className="container">
          <S.ProductBriefing>
            ...
            <S.ProductMeta>
              <S.ProducTitle>{product.name}</S.ProducTitle>
              <S.ProductMeta1>
                <S.ProductRating>
